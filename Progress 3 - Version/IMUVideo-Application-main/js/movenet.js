...
async function renderFrame() {
  ...
  const modeLabel = "Mode: Upload (MoveNet Thunder)";
  ...
  // Optional: Update IMU charts to follow video time (upload mode only).
  // By default we keep video + IMU timelines INDEPENDENT (S.followVideo=false).
  if (
    currentMode === "upload" &&
    S?.followVideo &&
    S.csvData &&
    S.csvTimesSeconds.length &&
    imuPanel &&
    imuPanel.style.display !== "none"
  ) {
    const now = performance.now();
    if (now - S.lastImuUpdateTime > S.IMU_UPDATE_THROTTLE_MS) {
      if (typeof IMU.updateChartsForVideoTime === "function") {
        IMU.updateChartsForVideoTime(video.currentTime || 0);
      }
      S.lastImuUpdateTime = now;
    }
  }
...
/*   if (currentMode === "live") {
    ...
    if (window.videoSpeedSelect) {
      const r = parseFloat(window.videoSpeedSelect.value);
...
/** 
 * Setup basic video playback controls: Play/Pause, Forward, Slower.
*/
function setupBasicVideoControls() {
  if (!videoPlayPauseBtn || !videoBackBtn || !videoForwardBtn || !videoSpeedSelect) return;

  function setEnabled(enabled) {
    videoPlayPauseBtn.disabled = !enabled;
    videoBackBtn.disabled = !enabled;
    videoForwardBtn.disabled = !enabled;
    videoSpeedSelect.disabled = !enabled;
  }

  function applySelectedSpeed() {
    const rate = parseFloat(videoSpeedSelect.value);
    if (Number.isFinite(rate)) video.playbackRate = rate;
  }

  function setPlayPauseUI(isPlaying) {
    if (window.videoPlayPauseIcon) window.videoPlayPauseIcon.textContent = isPlaying ? "⏸" : "▶";
    if (window.videoPlayPauseText) window.videoPlayPauseText.textContent = isPlaying ? "Pause" : "Play";
  }
  ...
  videoPlayPauseBtn.addEventListener("click", async () => {
    ...
    // If tracking is running, pause tracking + video
    if (isRunning) {
      stopTracking();
  ...
  // back: 1 step at VIDEO_TIMESTEP_FPS
  videoBackBtn.addEventListener("click", () => {
    if (!uploadedVideoReady) return;
    const step = 1 / VIDEO_TIMESTEP_FPS;
    const prev = Math.max(0, (video.currentTime || 0) - step);
    video.currentTime = prev;
  });

  // forward: 1 step at VIDEO_TIMESTEP_FPS
  videoForwardBtn.addEventListener("click", () => {
    if (!uploadedVideoReady) return;
    const step = 1 / VIDEO_TIMESTEP_FPS;
    const next = Math.min(video.duration || 0, (video.currentTime || 0) + step);
  ...
  // update play/pause UI on play/pause events
...
function handleFileChange() {
  ...
  video.onloadedmetadata = () => {
    ...
    if (typeof IMU?.setCurrentVideoFileInfo === "function") {
      IMU.setCurrentVideoFileInfo(file);
    }

    // Load any stored project (syncOffset + timestamps) for this video
    if (typeof IMU?.loadProjectFromLocal === "function") {
      IMU.loadProjectFromLocal();
    }

    // timeupdate: update slider & IMU charts even when MoveNet is stopped
    videoTimeUpdateListener = () => {
      if (!S?.csvData || !S.csvTimesSeconds.length) {
      ...
      // Only update IMU view from video when explicitly enabled.
      if (S?.followVideo && typeof IMU.updateChartsForVideoTime === "function") {
        IMU.updateChartsForVideoTime(t);
      }
    };
